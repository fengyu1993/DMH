function [S, Gamma, delta_q, error] = DMH_Method_KI_IK(S_init, Gamma_init, delta_q_init, q_bar_list, T_d_list, epsilon, tau_max, sigma, flag)
%% Damped Modified Halley Method for solving Kinematic Identification and Inverse Kinematics 
%% Input
...
for tau = 1 : tau_max
    ...
    for i = 1 : m
        ...
        J_xi_i = -get_dexp_inv(-xi_i) * AdjointInv(T_xi_i) * J_Phi;
        % Eq.(53)
        ...
        for k = 1 : l_Phi
            ...
            V_e_i = J_Phi * d_Phi_k;
            % Eq.(45)
            V_xi_i = -AdjointInv(T_xi_i) * V_e_i;
            % Eq.(55)
            d_J_Phi = get_d_J_Phi(S_tau, Gamma_tau, delta_q_tau, q_bar_list(:,i), d_S_tau , d_Gamma_tau, d_delta_q_tau, flag);
            d_J_xi_i = -get_dexp_inv_dt(-xi_i, -d_xi_i) * AdjointInv(T_xi_i) * J_Phi ...
                                - get_dexp_inv(-xi_i) * get_d_AdT_inv_dt(T_xi_i, V_xi_i) * J_Phi ...
                                        - get_dexp_inv(-xi_i) * AdjointInv(T_xi_i) * d_J_Phi;
            % Eq.(54)
            for j = 1 : 6
                H_Xi(:, k, 6*(i-1) + j) = d_J_xi_i(j, :)';
            end
        end
        % Eq.(22)(25)
        Xi(6*i-5 : 6*i, 1) = xi_i;
    ...
    % Eq.(39)
    for i = 1 : l_Xi
        temp_H(i, :) = Delta_Phi_DNR' * H_Xi(:,:,i);
    end
    J_Xi_DNR = J_Xi + 1/2*temp_H;
    Delta_Phi_DTH = -inv(J_Xi_DNR' * J_Xi_DNR + sigma*I) * J_Xi_DNR' * Xi;
    % Eq.(40)
    Phi_DTH = Phi + Delta_Phi_DTH;
    [S_DTH , Gamma_DTH, delta_q_DTH] = PhiToSGq(Phi_DTH, n, flag, S_init, Gamma_init, delta_q_init);
    Xi_DTH = get_Xi(S_DTH , Gamma_DTH, delta_q_DTH, q_bar_list, T_d_list);
    % Eq.(41)
    for i = 1 : l_Xi
        temp_H(i, :) = Delta_Phi_DTH' * H_Xi(:,:,i);
    end
    J_Xi_DTH = J_Xi + temp_H;
    Delta_Phi_DMH = -inv(J_Xi_DTH' * J_Xi_DTH + sigma*I) * J_Xi_DTH' * Xi_DTH;
    % Eq.(42)
    Phi = Phi_DTH + Delta_Phi_DMH;
end
%% Output
% Eq.(21)(24)
[S , Gamma, delta_q] = PhiToSGq(Phi, n, flag, S_init, Gamma_init, delta_q_init);
[S_end , Gamma_end, delta_q_end] = PhiToSGq(Phi, n, flag, S_init, Gamma_init, delta_q_init);
Xi_end = get_Xi(S_end , Gamma_end, delta_q_end, q_bar_list, T_d_list);
error(tau+1) = Xi_end' * Xi_end;
error = error(1 : tau+1);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function dJ_Phi_dt = get_d_J_Phi(S, Gamma, delta_q, q_bar, d_S, d_Gamma, d_delta_q, flag)
    ...
    if flag(3) == 1
        dt_Jdq = [];
        for k = 1 : n
            dt_Jdqk = dt_Ad_Ti_n1_inv(:, :, k+1) * dexp_list(:,:,k) * Slist(:, k)  ...
                + AdjointInv(Ti_n1_list(:, :, k+1)) * dt_dexp_list(:, :, k) * Slist(:, k) ...
                + AdjointInv(Ti_n1_list(:, :, k+1)) * dexp_list(:,:,k) * dSlist(:, k);
